...
def create_nodes(api, project):
    ...
    api.save(collection)
    expt = c.Experiment(name="Anionic Polymerization of Styrene with SecBuLi", collection=collection)
    api.save(expt)
    ...
    inventory = c.Inventory(collection=collection, name="Tutorial Materials", materials=materials)
    api.save(inventory)
    ...
    api.save(sec_data)
    file = c.File(project=project, data=[sec_data], name="sec_data", source=file_path + "\\sec.txt")
    api.save(file)
    sec_data.file = [file]
    api.save(sec_data)
    ...
    api.save(nmr_data)
    file = c.File(project=project, data=[nmr_data], name="nmr_data", source=file_path + "\\nmr.txt")
    api.save(file)
    sec_data.file = [file]
    api.save(sec_data)
